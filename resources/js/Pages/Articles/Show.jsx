        ...
        <PublicLayout>
                    ...
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-end', padding: 'clamp(30px, 6vw, 60px) 5vw' }}>
                        <div style={{ maxWidth: 800 }}>
                            <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
                                {article.category && <span style={{ padding: '5px 14px', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 50, fontSize: 12, color: '#fff', backdropFilter: 'blur(10px)' }}>{article.category}</span>}
                                <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>
                                    {article.published_at && new Date(article.published_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                                </span>
                                {article.author && <span style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)' }}>by {article.author.name}</span>}
                            </div>
                            <h1 className="font-syne" style={{ fontSize: 'clamp(1.8rem, 5vw, 3.5rem)', fontWeight: 800, color: '#fff', lineHeight: 1.1 }}>{article.title_en}</h1>
                        </div>
                    </div>
                </div>
            )}
                    ...
                    {article.tags && article.tags.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 40 }}>
                            {article.tags.map(tag => (
                                <span key={tag} style={{ padding: '5px 12px', border: '1px solid var(--border-glass)', borderRadius: 50, fontSize: 12, color: 'var(--text-secondary)' }}>#{tag}</span>
                            ))}
                        </div>
                    )}
            ...
            {related.length > 0 && (
                <section style={{ padding: 'clamp(40px, 6vw, 60px) 5vw', borderTop: '1px solid var(--border-glass)' }}>
                    <h3 className="font-syne" style={{ fontSize: 'clamp(1.3rem, 3vw, 1.8rem)', fontWeight: 700, textTransform: 'uppercase', marginBottom: 30 }}>More Articles</h3>
                        ...
                        {related.map(a => (
                            <Link key={a.id} href={`/articles/${a.slug}`} style={{ textDecoration: 'none', color: 'inherit', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--border-glass)', borderRadius: 14, overflow: 'hidden', transition: 'border-color 0.3s' }} onMouseOver={(e) => (e.currentTarget.style.borderColor = 'var(--accent)')} onMouseOut={(e) => (e.currentTarget.style.borderColor = 'var(--border-glass)')}>
                                ...
                                <div style={{ padding: 20 }}>
                                    ...
                                    <h4 className="font-syne" style={{ fontSize: 16, fontWeight: 700, lineHeight: 1.3 }}>{a.title_en}</h4>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <style>{`
                .prose h2 { font-size: 1.5rem; font-weight: 700; margin: 2.5rem 0 1rem; font-family: 'Syne', sans-serif; }
                .prose h3 { font-size: 1.3rem; font-weight: 600; margin: 2rem 0 0.8rem; font-family: 'Syne', sans-serif; }
