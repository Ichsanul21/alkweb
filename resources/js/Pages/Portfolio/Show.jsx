        ...
        <PublicLayout>
            ...
            {portfolio.featured_image && (
                <div style={{ width: '100%', height: 'clamp(300px, 50vh, 500px)', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ position: 'absolute', bottom: 'clamp(30px, 6vw, 60px)', left: '5vw', color: '#fff', zIndex: 2 }}>
                        <span style={{ display: 'inline-block', padding: '6px 16px', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 50, fontSize: 12, marginBottom: 12, backdropFilter: 'blur(10px)' }}>{portfolio.category}</span>
                        <h1 className="font-syne" style={{ fontSize: 'clamp(2rem, 6vw, 4rem)', fontWeight: 800, textTransform: 'uppercase', lineHeight: 1 }}>{portfolio.title_en}</h1>
                    </div>
                </div>
            )}
                ...
                <div style={{ maxWidth: 900, margin: '0 auto' }}>
                    ...
                    {portfolio.tags && portfolio.tags.length > 0 && (
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 40 }}>
                            {portfolio.tags.map(tag => (
                                <span key={tag} style={{ padding: '6px 14px', border: '1px solid var(--border-glass)', borderRadius: 50, fontSize: 12, color: 'var(--text-secondary)' }}>{tag}</span>
                            ))}
                        </div>
                    )}
                    ...
                    {portfolio.gallery && portfolio.gallery.length > 0 && (
                        <div style={{ marginTop: 60 }}>
                            <h3 className="font-syne" style={{ fontSize: 20, fontWeight: 700, marginBottom: 24 }}>Gallery</h3>
            ...
            {related.length > 0 && (
                <section style={{ padding: 'clamp(40px, 6vw, 60px) 5vw', borderTop: '1px solid var(--border-glass)' }}>
                    <h3 className="font-syne" style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)', fontWeight: 700, textTransform: 'uppercase', marginBottom: 30 }}>Related Projects</h3>
                        ...
                        {related.map(p => (
                            <Link key={p.id} href={`/portfolio/${p.slug}`} style={{ textDecoration: 'none', position: 'relative', borderRadius: 14, overflow: 'hidden', background: '#000', display: 'block', height: 280 }}>
                                ...
                                <div style={{ position: 'absolute', bottom: 20, left: 20, color: '#fff' }}>
                                    <span style={{ display: 'inline-block', padding: '4px 10px', border: '1px solid rgba(255,255,255,0.3)', borderRadius: 50, fontSize: 11, marginBottom: 6 }}>{p.category}</span>
                                    <h4 className="font-syne" style={{ fontSize: 18, fontWeight: 700 }}>{p.title_en}</h4>
                                </div>
                            </Link>
                        ))}
                    </div>
            ...
            <style>{`
                .prose h2 { font-size: 1.5rem; font-weight: 700; margin: 2rem 0 1rem; font-family: 'Syne', sans-serif; }
                .prose h3 { font-size: 1.25rem; font-weight: 600; margin: 1.5rem 0 0.8rem; font-family: 'Syne', sans-serif; }
